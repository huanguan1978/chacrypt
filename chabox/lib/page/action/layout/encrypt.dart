...
import '../../../utils/crypt_helper.dart' show CommandException, encryptCommand;
...
import '../widget/crypt.dart' as widget;

class Encrypt extends StatefulWidget with WatchItStatefulWidgetMixin {
...
class _EncryptState extends State<Encrypt> {
  ...
  void _dataCallback(List<(String, String)> datas) {
    if (datas.isNotEmpty) {
      _logStreamController.add(ML.INFO, 'Encrypted: $datas');
    }
    setState(() {
      _isProcessing = false;
    });
  ...
  void initState() {
    ...
    _selectedOutDir = caching.encpath.value;
    ...
    _encryptCommandResultsListen = encryptCommand.results.listen((rst, _) {
      ...
      final isConfigDir = isWithinPath(caching.encpath.value, _selectedOutDir);
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _logStreamController.add(ML.INFO, ME.tr(MD.stEncryptGlobalInfoAtomic));
  ...
  void _encrypt() {
    if (_paths.isEmpty) {
      _logStreamController.add(ML.WARNING, 'No files selected for encryption.');
      return;
    }

    // Pre-flight check for directories (Atomic Security Policy)
    ...
    if (hasDirectory) {
      _logStreamController.add(
        ML.SEVERE,
        "Operation Interrupted: Directory detected in source list. For security, this module does not generate temporary archives. Please pack directories manually.",
      );
      return;
    }
    ...
    _logStreamController.add(
      ML.INFO,
      'Starting atomic encryption of ${_paths.length} files to $_selectedOutDir',
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ActionLayout(
        ...
        actionWidget: widget.CryptActionWidget(
          isDesktop: isDesktop,
          type: widget.CryptActionType.encrypt,
