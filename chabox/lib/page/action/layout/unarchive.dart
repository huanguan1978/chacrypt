...
import '../../../../utils/crypt_helper.dart'
    show CommandException, unarchiveCommand;
...
import '../widget/unarchive.dart' as widget;

class Unarchive extends StatefulWidget with WatchItStatefulWidgetMixin {
  const Unarchive({super.key});

  @override
  State<Unarchive> createState() => _UnarchiveState();
}

class _UnarchiveState extends State<Unarchive> {
  ...
  void _dataCallback(List<(String, String)> datas) {
    if (datas.isNotEmpty) {
      _logStreamController.add(ML.INFO, 'Unarchived: $datas');
    }
    setState(() {
      _isProcessing = false;
    });
  ...
  void initState() {
    ...
    _selectedOutDir = caching.deapath.value;
    ...
    _unarchiveCommandResultsListen = unarchiveCommand.results.listen((rst, _) {
      ...
      final isConfigDir = isWithinPath(caching.deapath.value, _selectedOutDir);
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _logStreamController.add(ML.INFO, ME.tr(MD.stUnarchiveGlobalInfoSupport));
  ...
  void _unarchive() {
    if (_paths.isEmpty) {
      _logStreamController.add(
        ML.WARNING,
        'No files selected for unarchiving.',
      );
      return;
    }
    ...
    _logStreamController.add(
      ML.INFO,
      'Starting unarchive of ${_paths.length} items to $_selectedOutDir',
    );
  ...
  Widget build(BuildContext context) {
    ...
    watchValue((Caching c) => c.deapath);
