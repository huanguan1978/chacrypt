...
class SettingDecryptedFolder extends StatelessWidget with WatchItMixin {
  const SettingDecryptedFolder({super.key});

  @override
  Widget build(BuildContext context) {
    final isDesktop = sl<bool>(instanceName: 'isDesktop');
    final decpath = watchValue((Caching c) => c.decpath);
      ...
      options: [
        ...
        if (isDesktop)
          DirectoryOption(
            ...
            onAction: () => DirectorySelector.openDirectory(decpath),
          ),
      ],
      onChanged: (l, _) => sl<Caching>().decpath = l,
    );
  }
}
